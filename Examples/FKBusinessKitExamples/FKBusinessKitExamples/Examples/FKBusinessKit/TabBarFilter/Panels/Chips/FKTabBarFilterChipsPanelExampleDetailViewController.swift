...
final class FKTabBarFilterChipsPanelExampleDetailViewController: UIViewController {

  private let exampleCase: FKTabBarFilterChipsPanelExampleCase
  private let logView = UITextView()
  private let panelContainer = UIView()

  init(exampleCase: FKTabBarFilterChipsPanelExampleCase) {
  ...
  override func viewDidLoad() {
    ...
    let panel = FKTabBarFilterChipsViewController(
      sections: spec.sections,
      configuration: spec.configuration,
      onChange: { [weak self] updated in
        self?.appendLog("onChange([FKTabBarFilterSection]) count=\(updated.count)")
      },
