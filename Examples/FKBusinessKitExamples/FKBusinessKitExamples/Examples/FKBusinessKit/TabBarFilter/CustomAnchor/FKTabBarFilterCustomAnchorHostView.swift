...
/// `FKTabBar` stays in the hierarchy (required by `FKTabBarFilterDropdownController`) but is laid out **off-screen**
/// so it does not appear in the UI — interaction uses `open` / `close` / `toggle` from code instead of tab taps.
final class FKTabBarFilterCustomAnchorHostView: UIView, FKTabBarFilterTabBarHost {
  /// User-visible bar; also the ``FKTabBarFilterDropdownController/setAnchor(source:overlayHost:)`` source view.
  ...
  private func commonInit() {
    ...
    NSLayoutConstraint.activate([
      // Same horizontal placement and minimum height as `FKTabBarFilterTabBarHostView`’s `FKTabBar` row.
