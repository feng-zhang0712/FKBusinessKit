...
/// Shared chrome for filter-strip examples: **one** top bar (`FKTabBar` + divider).
///
/// - **Tab-bar anchor demo**: uses the default anchor (``FKTabBar``).
/// - **Custom-anchor demo**: calls ``FKTabBarFilterController/setAnchor(source:overlayHost:)`` on a separate control.
///   wrapper view instead of `tabBar` — still a **single** on-screen bar (no extra “strip” above tabs).
final class FKTabBarFilterTabBarHostView: UIView, FKTabBarFilterTabBarHost {
  let tabBar: FKTabBar = {
    let bar = FKTabBar()
