...
final class FKTabBarFilterSingleListPanelExampleDetailViewController: UIViewController {

  private let exampleCase: FKTabBarFilterSingleListPanelExampleCase
  private let logView = UITextView()
  private let panelContainer = UIView()

  init(exampleCase: FKTabBarFilterSingleListPanelExampleCase) {
  ...
  override func viewDidLoad() {
    ...
    let panel = FKTabBarFilterSingleListViewController(
      section: spec.section,
      configuration: spec.configuration,
      onChange: { [weak self] _ in self?.appendLog("onChange(FKTabBarFilterSection)") },
