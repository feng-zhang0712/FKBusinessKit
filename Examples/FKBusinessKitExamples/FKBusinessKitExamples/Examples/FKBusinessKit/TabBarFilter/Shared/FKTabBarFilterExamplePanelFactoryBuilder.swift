...
/// Builds a ``FKTabBarFilterPanelFactory`` bound to a shared ``FKTabBarFilterExampleState``.
enum FKTabBarFilterExamplePanelFactoryBuilder {
  @MainActor
  static func makeFactory(
    bindingTo state: FKTabBarFilterExampleState,
    loadingTitle: String = FKTabBarFilterExampleAppearance.panelLoadingTitle,
    wrapsPanelWithTopHairline: Bool = true,
    onTagsSelectionEmptied: (() -> Void)? = nil,
    includesCustomPromoPanel: Bool = false
  ) -> FKTabBarFilterPanelFactory {
    var sources: [FKTabBarFilterPanelKind: FKTabBarFilterPanelFactory.PanelSource] = [
    ...
    if includesCustomPromoPanel {
      sources[.custom("promo")] = .custom { _, onSelection in
        FKTabBarFilterCustomPanelKindExampleViewController.make(
          allowsMultipleSelection: false,
          onSelection: onSelection
        )
      }
    }
    return FKTabBarFilterPanelFactory(
      sourcesByPanelKind: sources,
      loadingTitle: loadingTitle,
      wrapsPanelWithTopHairline: wrapsPanelWithTopHairline
    )
  }
}
