...
/// switch tabs while expanded, or dismiss via backdrop / swipe when enabled in ``FKTabBarFilterConfiguration/presentationConfiguration``.
///
/// Factory-backed panels use ``FKTabBarFilterPanelFactory``; custom panels use ``FKTabBarFilterTabPanelContent``.
@MainActor
public final class FKTabBarFilterController<TabID: Hashable>: UIViewController {
  ...
  /// Optional factory for tabs whose ``FKTabBarFilterTab/panelContent`` is ``FKTabBarFilterTabPanelContent/panelKind``.
  ...
  public init(
    ...
    self.tabs = Self.makeResolvedTabs(
      tabs: tabs,
  ...
  /// Updates configuration and strip defaults without changing the tab list.
  ///
  /// Tab strip typography or panel wiring changes require ``replaceTabs(_:)`` so strip items are rebuilt.
  ...
  /// Applies anchor source, overlay host, and host strategy to this controller.
  ...
  private func updateResolvedTabs(_ tabs: [FKTabBarFilterResolvedTab<TabID>]) {
    self.tabs = tabs
    rebuildTabBarItems(keepSelectedTab: selectedTabInternal)
  ...
  ///
  /// No-op until ``setAnchor(source:overlayHost:)`` or ``FKTabBarFilterConfiguration/anchorPlacement`` is set.
  ...
  private var tabs: [FKTabBarFilterResolvedTab<TabID>] = []
  ...
  private func resolveContentController(for tab: FKTabBarFilterResolvedTab<TabID>) -> UIViewController {
    switch configuration.contentCachingPolicy {
    case .recreate:
  ...
  private func makeContentController(for tab: FKTabBarFilterResolvedTab<TabID>) -> UIViewController {
    switch tab.content {
  ...
  private static func makeResolvedTabs(
    tabs: [FKTabBarFilterTab<TabID>],
    panelFactory: FKTabBarFilterPanelFactory,
    ...
    tabs.map { tab in
      ...
      let strip = tab.tabStrip ?? defaultTabStrip
      ...
      return FKTabBarFilterResolvedTab.chevronTitle(
        ...
        normalTitleColor: strip.normalTitleColor,
        expandedTitleColor: strip.expandedTitleColor,
        normalChevronColor: strip.normalChevronColor,
        expandedChevronColor: strip.expandedChevronColor,
        titleFont: UIFont.preferredFont(forTextStyle: strip.titleTextStyle),
        subtitleFont: UIFont.preferredFont(forTextStyle: strip.subtitleTextStyle),
        chevronSize: strip.chevronSize,
        chevronSpacing: strip.chevronSpacing,
        titleSubtitleSpacing: strip.titleSubtitleSpacing,
...
// MARK: - Deprecated public names

/// Renamed to ``FKTabBarFilterController`` (same type).
@available(*, deprecated, renamed: "FKTabBarFilterController")
public typealias FKTabBarFilterDropdownController<TabID> = FKTabBarFilterController<TabID> where TabID: Hashable
