...
///
/// Layout:
/// - Left: `FKTabBarFilterTwoColumnModel.categories`
...
/// - Optional **section collapse**: ``FKTabBarFilterTwoColumnRightSectionHeaderBehavior/togglesSectionCollapse`` toggles
///   ``FKTabBarFilterSection/isCollapsed`` from the header; default collapsed state is stored per section in the model.
/// - Single-select scope (``FKTabBarFilterTwoColumnSingleSelectionScope``; typealias ``SingleSelectionScope``):
///   - ``FKTabBarFilterTwoColumnSingleSelectionScope/withinSection``: single selection only affects the tapped section
///   - ``FKTabBarFilterTwoColumnSingleSelectionScope/globalAcrossSections``: single selection clears other sections (useful for "course catalog" style)
/// - Height controlled via `Configuration.heightBehavior`
public final class FKTabBarFilterTwoColumnListViewController: UIViewController {
  ...
  public struct RightGroupedTableConfiguration {
    ...
    /// Background for the custom interactive right header (when ``Configuration/rightSectionHeaderBehavior`` is not ``FKTabBarFilterTwoColumnRightSectionHeaderBehavior/standard``).
  ...
  public struct Configuration {
    ...
    public var rightCellStyle: FKTabBarFilterListCellStyle
    ...
    public init(
      ...
      rightCellStyle: FKTabBarFilterListCellStyle = .init(),
  ...
  /// Preferred height from current model (left row count vs right rows/headers), after ``FKTabBarFilterPanelHeightBehavior``.
  ...
  private func section(at index: Int) -> FKTabBarFilterSection? {
    let sections = rightSections()
  ...
  /// Header tap handling aligned with ``FKTabBarFilterTwoColumnGridViewController`` (collapse vs selection are mutually exclusive modes).
  private func handleRightSectionHeaderTap(sectionIndex: Int) {
    ...
    let headerItem = FKTabBarFilterOptionItem(id: target.id, title: headerTitle, isSelected: true)
    onSelection?(.init(sectionID: target.id, item: headerItem, effectiveSelectionMode: .single))
  }
}

extension FKTabBarFilterTwoColumnListViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    let header = FKTabBarFilterTwoColumnListSectionHeaderView()
