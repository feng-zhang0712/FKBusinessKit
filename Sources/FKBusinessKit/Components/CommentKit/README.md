...
- Background tap dismisses keyboard: ``FKCommentKitConfiguration/dismissesKeyboardOnBackgroundTap`` (default `true`). When row-tap reply is on, taps that begin a reply keep the composer focused instead of dismissing.
...
- Compact composer chrome: ``FKCommentComposerConfiguration`` (`usesCapsuleInput`, `showsSendButton`, `showsReplyTargetBanner`, `showsCancelReplyButton`, capsule colors, …). Send stays disabled until the input has non-empty trimmed text. With ``FKCommentListViewController``, keep ``showsSendButton`` `true` — that is the only submit path.
- Composer presentation: ``FKCommentComposerPresentationMode`` — ``always`` (default), ``onDemand`` (show while composing), ``automatic`` (stay visible with text/drafts). Blur clears the reply stripe and visible text by default (`clearsCompositionOnBlur`); drafts are preserved per reply target (`preservesDrafts`) and restored on the next ``beginReply(to:)`` / ``beginTopLevelComment()``.
